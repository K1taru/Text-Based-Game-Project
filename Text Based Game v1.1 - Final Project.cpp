...
//Created: 11/10/2023 - 
//Text Based Game v1.1 - Final Project
...
void printLoading(int printTime, int delayTime, string text, bool pStats, int sleepTime)
{
	for(int i=0;i<3;i++)
	{
		if(pStats == true)
...
void createCharacter()
{
	...
	printLoading(300, 2000, "Creating Character", false, 0);
}
...
int main()
{
	while(userPlaying()) 
	{
		...
		printStats(HP,MP, 1000);
		printStats(HP-=10,MP, 1000);
		printStats(HP-=10,MP, 1000);
		printLoading(300, 1000, "Attacking the goblins", true, 0);
		printStats(HP-=10,MP, 1000);
		printStats(HP-=10,MP, 1000);
		printStats(HP-=10,MP, 1000);
		printStats(HP-=10,MP, 1000);
